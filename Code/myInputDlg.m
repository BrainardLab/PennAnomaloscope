...
validateModalDialogsCapability();
...
if (rw == 1 & cl == 2) %#ok Handle []
    % For 1X2 numLines, the value is repeated for NumQuest times
...
QuestHandle = gobjects(1,NumQuest);
EditHandle = gobjects(1,NumQuest);
...
for lp=1:NumQuest
    if isstring(DefAns{lp})
        DefAns{lp} = char(DefAns{lp});
    end
    if ~ischar(DefAns{lp})
        delete(InputFig);
...
% Callback to handle Enter key presses in Edit fields
function doEditCallback(obj,~)
% Keeping the first check because pressing 'Tab' after 
% editing an edit field also fires the callback.
% Regarding second check, for multiline input field, ARIA guidelines says 
% that the form is not submitted on pressing 'Enter'
% https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Roles/textbox_role
% Checking if the edit field is multiline.
% 'Max' property is set to be equal to numLines for inputdlg.
% Keeping 'Min' for any future modification.
if strcmp(get(gcbf, 'CurrentKey'),'return') && (obj.Max - obj.Min == 1)
